...
# best subset algo
model_subset <- regsubsets(log(y_train) ~ Backlog:Team_exp+ Backlog:G1+Backlog:G2+Backlog+Team_exp + Proc_chg +X1 + X2 + X3 + X4 + X5 + G1+ G2,
...
calculate_press <- function(x_train, y_train, model_subset, summary_subset) {
  ...
  for (i in 1:p) {
    indices <- which(summary_subset$which[i, ] != 1)
    predictors <- names(indicies) #p=i時用到的predictor
    ...
    for (j in 1:n) {
      ...
      model_loo <- lm(y_loo ~ x_loo)
      x_test <- x_selected[j, , drop = FALSE]
      y_pred <- predict(model_loo, newdata = as.data.frame(t(x_test)))
...
# PRESS 
plot(press, xlab = "Number of Predictors", ylab = "PRESS", type = "b", col = "brown")
title(main = "PRESS vs Number of Predictors")

par(mfrow = c(1, 1))
dev.off()

#選定較佳的模型:p=9,10,12,13
#model_1_9
model_1_9 <-lm(log(y_train)  ~ Backlog:G2 + Proc_chg + X1 + X2 + X4 + X5 + G1+ G2, data = x_train)
model_1_10 <-lm(log(y_train) ~ Backlog:G1+Backlog:G2+Backlog+ Proc_chg + X1 + X2 + X5 + G1+ G2, data = x_train)
model_1_12 <-lm(log(y_train) ~ Backlog:Team_exp+ Backlog:G1+Backlog:G2+Backlog + Proc_chg + X1 + X2 + X4 + X5 + G1+ G2, data = x_train)
model_1_13 <-lm(log(y_train) ~ Backlog:Team_exp+ Backlog:G1+Backlog:G2+Backlog+ Proc_chg + X1 + X2 + X3 + X4 + X5 + G1+ G2, data = x_train)

#存下model_1_9之summary table
model_1_9_df <- as.data.frame(summary(model_1_9)$coefficients)
addWorksheet(wb, "model_1_9") 
writeData(wb, "model_1_9", model_1_9_df) 
saveWorkbook(wb, criterion_xlsx, overwrite = TRUE)
...
#存下model_1_12之summary table
model_1_12_df <- as.data.frame(summary(model_1_12)$coefficients)
addWorksheet(wb, "model_1_12") 
writeData(wb, "model_1_12", model_1_12_df) 
saveWorkbook(wb, criterion_xlsx, overwrite = TRUE)

#存下model_1_13之summary table
model_1_13_df <- as.data.frame(summary(model_1_13)$coefficients)
addWorksheet(wb, "model_1_13") 
writeData(wb, "model_1_13", model_1_13_df) 
saveWorkbook(wb, criterion_xlsx, overwrite = TRUE)
